...
for input_file in input_files:
  ...
  if status: print 'file:', input_file, 'error:', status
    ...
    if save_curves:
      if x is None: np.savetxt(output_dir + '/' + basename + '.txt', y, fmt='%s', delimiter='\t')
      else:         np.savetxt(output_dir + '/' + basename + '.txt', np.transpose(np.array([x, y])), fmt='%s', delimiter='\t')
    if save_steps: np.savetxt(output_dir + '/steps/' + basename + '.txt', meta['steps'], fmt=('%d','%s'), delimiter='\t')
