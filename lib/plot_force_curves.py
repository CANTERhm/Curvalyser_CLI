...
from CurvalyserLib import load_curvalyser_curve_data, load_curvalyser_step_data, curve_file_fields, step_file_fields, \
    load_curve, prepare_data, denoise, nat2py_index, fmt_float
...
def plot_curve(x, y, y_denoised, x_trace, y_trace, steps, filtered_steps, c, filename):
    if c['plot_size_force_curves'] is None:
        ...
        fig_size = (float(c['plot_size_force_curves'][0]) / pl.rcParams['savefig.dpi'],
                    float(c['plot_size_force_curves'][1]) / pl.rcParams['savefig.dpi'])
...
parser.add_option('-C', '--config_file', dest='config_file', metavar='FILE',
                  help='set Paramalyser config file (default: config-paramalyser)')
parser.add_option('-c', '--cfg_filter', dest='config_filter', metavar='CODE',
                  help='Python expression to select config files')
parser.add_option('-e', '--exp_types', dest='exp_types', metavar='LIST',
                  help='only include listed experiment types (comma-separated list)')
parser.add_option('-f', '--files', dest='file_pattern', metavar='FILE_PATTERN',
                  help='only include force curves matching FILE_PATTERN')
parser.add_option('-r', '--range', dest='file_range', metavar='FILE_RANGE',
                  help='select range of curves (start:stop:step,start:stop:step,...)')
parser.add_option('-o', '--output_dir', dest='base_output_dir', metavar='DIR', help='Curvalyser output directory')
parser.add_option('-p', '--plots_dir', dest='plots_dir', metavar='DIR',
                  help='directory for plots (default: plots_filtered)')
...
if options.config_file is not None:
    ...
    exit(1)
if options.plots_dir is None:
    plots_dir = 'plots_filtered'
else:
...
for config_file_no, config_file in enumerate(config_files):
    ...
    if exp_types is not None and exp_type is not None and exp_type not in exp_types:
        ...
        exp_id, float(config_file_no) / len(config_files) * 100., config_file_no + 1, len(config_files), exp_type)
        continue
    ...
    if c['file_range'] is None:
        ...
        files = get_slice(files, c['file_range'])
    if not len(files):
    ...
    if curves is None or steps is None:
        print '%s (%.1f%%, exp. %d of %d, %s):\t%d file(s)' % (
        exp_id, float(config_file_no) / len(config_files) * 100., config_file_no + 1, len(config_files), exp_type,
        len(files))
        if os.path.exists(c['output_dir']):
            print "[warning] no Curvalyser data in directory '%s'" % c['output_dir']
        else:
            print "[warning] Curvalyser data directory '" + c['output_dir'] + "' doesn't exist"
        ...
        if len(step_filter_curve_limits) or len(
            step_filter_step_limits):  filtered_curves, filtered_steps = filter_steps(filtered_curves, filtered_steps,
                                                                                      step_filter_curve_limits,
                                                                                      step_filter_step_limits)
        if len(curve_filter_curve_limits) or len(
            curve_filter_step_limits): filtered_curves, filtered_steps = filter_curves(filtered_curves, filtered_steps,
                                                                                       curve_filter_curve_limits,
                                                                                       curve_filter_step_limits)
        print '%s (%.1f%%, exp. %d of %d, %s):\t%d file(s), %d/%d curve(s), %d/%d step(s)' % (
        exp_id, float(config_file_no) / len(config_files) * 100., config_file_no + 1, len(config_files), exp_type,
        len(files), curves.size, filtered_curves.size, steps.size, filtered_steps.size)
    ...
    for file_no, filename in enumerate(files):
        ...
        if status:
            print '  %s / %s (%5.1f%%, exp. %3d of %3d, file %3d of %3d): skipped' % (
            exp_id, basename, (float(config_file_no) + float(file_no + 1) / len(files)) / len(config_files) * 100,
            config_file_no + 1, len(config_files), file_no + 1, len(files))
            continue
        ...
        if curve_numbers.size == 1:
            ...
            selected_filtered_steps = filtered_steps[
                filtered_steps.curve_no == filtered_curve_numbers[0]] if filtered_curve_numbers.size == 1 else np.array(
                [])
            print u'  %s / %s (%5.1f%%, exp. %3d of %3d, file %3d of %3d): %2d/%2d step(s), noise sigma: %6.2f %s, denoising: %s, ind thld: %6.2f %s/%s' % (
            exp_id, basename, (float(config_file_no) + float(file_no + 1) / len(files)) / len(config_files) * 100,
            config_file_no + 1, len(config_files), file_no + 1, len(files), selected_steps.size,
            selected_filtered_steps.size, curve['noise_sigma'], c['unit_y'],
            fmt_float(curve['denoising_param'], '%6.2f'), curve['indicator_thld'], c['unit_y'], c['unit_x'])
            if options.verbose:
                for i, step in enumerate(
                    selected_steps): print '    step #%2d: extension: %7.3f %s, height: %7.3f %s, avg slope: %7.3f %s/%s, max slope: %7.3f %s/%s%s' % (
                i + 1, step['extension'], c['unit_x'], step['height'], c['unit_y'], step['avg_slope'], c['unit_y'],
                c['unit_x'], step['max_slope'], c['unit_y'], c['unit_x'],
                '' if step['lmax_pos'] in filtered_steps.lmax_pos else ' (filtered)')
            ...
            print '  %s / %s (%5.1f%%, exp. %3d of %3d, file %3d of %3d): no Curvalyser results (%d matches)' % (
            exp_id, basename, (float(config_file_no) + float(file_no + 1) / len(files)) / len(config_files) * 100,
            config_file_no + 1, len(config_files), file_no + 1, len(files), curve_numbers.size)
            if y is not None: y -= np.mean(y[-y.size / 10:])
            if y_trace is not None: y_trace -= np.mean(y_trace[-y_trace.size / 10:])
            if x is not None: x -= x[0]
            if x_trace is not None: x_trace -= x_trace[0]
            y_denoised = y
        plot_curve(x, y, y_denoised, x_trace, y_trace, selected_steps, selected_filtered_steps, c,
                   c['output_dir'] + '/' + plots_dir + '/' + basename + '-force')
    print '  %d/%d curve(s), %d/%d adhesion curve(s), %d/%d step(s), execution time = %s\n' % (
    curves.size, filtered_curves.size, np.sum(curves.steps_ctr > 0), np.sum(filtered_curves.steps_ctr > 0), steps.size,
    filtered_steps.size, datetime.datetime.now() - t1)
    total_experiments += 1
...
if len(
    config_files) > 1: print '%d experiment(s), %d/%d curve(s), %d/%d adhesion curve(s), %d/%d step(s), execution time = %s' % (
total_experiments, sum(total_curves.values()), sum(total_curves_filtered.values()), sum(total_adhesions.values()),
sum(total_adhesions_filtered.values()), sum(total_steps.values()), sum(total_steps_filtered.values()),
